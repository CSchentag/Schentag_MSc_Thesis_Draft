...
The SolidWorks thermal analysis software uses Finite Element Thermal Analysis to solve simulations, and considers heat flux due to heat power, radiation, conduction and convection. The unknown in these equations is the temperature. The restraints on these equations are the specified temperatures (for example, the baseplate must be -40°C as a worst-case scenario), and the reactions are the resultant heat flow that is necessary to maintain the specified temperatures. All other conditions add source terms, such as the dissipated heat power of electrical components~\cite{FEA_SW}.
